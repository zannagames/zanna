...
| GAP-9 | Graphics3D | Narrowed 2026-08-13: the OpenGL backend implements `Canvas3D.FrameGpuTimeUs` via an `ARB_timer_query` `GL_TIMESTAMP` ring, so Linux now reports GPU frame time. The value trails by up to `VGFX3D_GL_TIMER_FRAMES` (3) frames and reads 0 until the first pair retires or when the extension is unavailable — polling never blocks the pipeline it measures. Still 0 on macOS: Metal has no `get_frame_gpu_time_us` hook | Low |

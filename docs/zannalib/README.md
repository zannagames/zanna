...
| [Platform Services](services.md) | `Platform`, `Request`, `Achievements`, `Stats`, `Leaderboards`, `Presence`, `Overlay`, `OnScreenKeyboard`, `Cloud`, `Timeline`, `Steam`, and constants — store identity, licensing, DLC, events, achievements, stats, leaderboards, presence, overlay, keyboards, cloud files, and recording timeline markers (Steam first) |

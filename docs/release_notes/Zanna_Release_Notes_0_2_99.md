...
- **Xenoscape rebuilt from scene files (new).** The demo campaign's ten regions moved out of about 3,500 lines of level-builder code into authored scene files that a 120-line loader reads. A parity tool proves the scene-driven game matches the original tile for tile and spawn for spawn, so the scenes — not the code — are the source of truth from here on.
...
- The scene-driven Xenoscape carries the original game's fourteen probes over unchanged and passes them against scene-loaded levels, and a parity probe checks every region against the code-built campaign tile for tile, spawn for spawn, and interaction for interaction.
...
Xenoscape now ships twice. `examples/games/xenoscape` is the original, built by code; `examples/games/xenoscape-scenes` plays identically but loads all ten regions from authored `.scene` files you can open in Studio, edit, and launch with Run Scene. It's the worked example for the scene pipeline, complete with tiles, sprites, sound, UI, packaging, and an asset library, and the exporter that produced it — along with the parity probe that proves the two agree — lives in the original's `tools/` directory.

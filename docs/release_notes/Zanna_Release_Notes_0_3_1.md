...
The release also makes secondary camera views more practical. A picture-in-picture, security monitor, or live in-world display can reuse shadows that are still valid from the main camera, and can limit the shadow work it requests when that is the right trade-off. That gives projects more room for these views without taking time away from the main frame.
...
- `Material3D` supports projected decals: a texture can be projected onto a model for markings such as uniforms, signage, logos, or wear. The decal follows animated geometry and is lit with the material beneath it across supported renderers.
...
- The standard build scripts preserve CTest's learned timing information and seed known serialized test lanes on a fresh build. In a representative 2,004-test `-j10` run, this reduced elapsed time from 554 seconds to 291 seconds while retaining the same test selection.
- Fast Debug builds now optimize the C runtime while preserving assertions and debug information, substantially reducing the cost of software-rendered and headless graphics checks.

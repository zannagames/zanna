...
`package-icon` may be repeated, one square PNG per line, each at least 16x16 with at least one source of 32x32 or larger; every line adds one size, and repeating a path or a pixel size is an error. Each platform icon size (macOS `.icns` 16-1024, Windows `.ico` 16-256, Linux hicolor 16-256) reuses the source whose size matches exactly, byte for byte (Windows `.ico` entries re-encode a PNG that is not 8-bit RGBA), and otherwise shrinks the smallest larger source with an alpha-weighted area filter, or enlarges the largest source when none is larger; macOS packages warn when the largest source is under 1024x1024. The `.icns` carries Apple's full record set, including the 16 and 32 pixel `ARGB` records non-Retina displays use. Resizing is integer-exact, so every host produces identical icons ([ADR 0372](../adr/0372-package-icon-sources-and-dmg-volume-icons.md)).
...
| `--macos-dmg-icon <path>` | Volume icon for the generated toolchain `.dmg`: a `.icns` file or a square `.png`; defaults to the Zanna mark |

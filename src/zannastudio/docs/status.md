...
live push/pull output, explicit integration strategies, side selection, Stage
confirmation, continue/skip/abort recovery, and external credential brokers.
It still lacks stash management and a graphical three-pane merge editor. Scene
...
truthfully (ADR 0191; deterministic probes still use the software path).
Metal uses a layer-free device context, Linux OpenGL uses a surfaceless EGL
pbuffer, and D3D11 uses a device-only hardware/WARP context, so accelerated
offscreen rendering no longer requires a hidden platform window. Interactive
...
| Scene files | Implemented with limits | `.scene2d` mounts the 2D editor and `.scene3d` mounts the 3D editor (legacy `.scene`/`.level`/`.vscn` remain accepted). Both retain per-document workspace/history state and provide real expandable multi-select hierarchies, transactional before/into/after row drops, group edits, typed gameplay data, searchable project assets, hierarchy-preserving clipboard transfer, undo/redo, and safe save/import flows. Both editors present real icon toolbars (ADR 0220): the scene vector-icon set covers tools, primitives, node kinds, and view chrome; tool modes are checked toggles instead of state-in-label buttons; hidden items collapse without leaving spacing; and the stateful command menus anchor to their toolbar items' reported geometry. Component inspectors follow the ADR 0221 live-commit contract: state-shaped fields (2D camera/lighting, object type/id, draw-stack order; 3D light/camera/collider/material singles) commit one undoable edit per completed gesture with labeled inputs, while creation, batch, fork, bake, and library actions keep explicit verb buttons. At every width, complete stateful 2D Scene/Tools/View and 3D Scene/Create/Placement/View menus keep secondary commands out of permanent chrome; wide lanes grow the authored canvas/viewport instead of restoring every historical button, while active tools, Game View, and contextual pane navigation remain direct. Each Scene inspector also exposes one per-document Topic at a time—six complete 2D topics or four complete 3D topics—so unrelated setup, history, lighting, assets, layers, and metadata do not form one endless page. The 2D surface includes a runtime-backed organizational hierarchy with absolute positions, one-step root/child creation, explicit cycle-safe multi-root reparenting, stable subtree/sibling ordering, real bounded atlas rendering/palettes, viewport-windowed canvas rasterization (only visible cells render, so large maps stay fully scroll-reachable; 100% zoom equals the authored tile resolution, the wheel zooms around the pointer, and Fit/1:1/grid-toggle controls remain reachable through View actions), captured gap-free paint/erase with exact cancellation, inclusive rectangle paint, four-connected fill, active-layer tile picking, modifier-aware point and inclusive authored-cell marquee selection, object dragging, scene/object properties, nudging, alignment, and distribution. The 3D surface includes a runtime-backed shaded/triangle-wireframe viewport with exact editor-overlay alignment, exact preserve-world chooser/direct reparenting with preserve-local opt-out, stable sibling ordering, mixed-state batch visibility, switchable Local/World Move/Rotate/Scale with snapping and atomic exact-or-reject world conversion, filled Move-plane and crossed Scale-plane XY/XZ/YZ handles, projected X/Y/Z rotation rings with wrap-safe angular dragging, truthful mixed-value batch PBR materials, batch embedded texture maps, mixed-value batch authoring for every runtime light type with hierarchy/viewport feedback, authored camera nodes with look-through and a bounded preview inset, collider-convention authoring with wireframe overlays, exact canonical-mesh terrain creation and four-mode viewport sculpting, and route polylines/badges for project gameplay components. Both load compatible definitions from bounded root-local `scene-components.json`; Add Missing preserves same-kind values, rejects any type conflict before mutation, and commits the complete selection once. A shared structured form maintains the complete cross-target schema through parser-validated atomic writes, external-conflict detection, and separate bounded file undo/redo. The 2D tile palette authors per-tile collision, typed int/bool properties, per-frame animations, and 16-variant autotile rules as one-transaction typed-section edits with palette behavior badges. Schema v19 adds typed Scene Settings forms (ADR 0222): scene-level components bind root metadata (3D) and scene properties (2D) to grouped labeled forms with color swatches, bounded sliders, angle/vec fields, labeled int-stored enums, ref fields, and bounded repeat groups with one-transaction add/remove — ashfall's 78 mission root keys and xenoscape's region settings author as typed forms while the raw grids stay authoritative for undeclared keys. The 3D viewport readback pipeline reuses one retained buffer (zero transient allocations on steady frames) and reports a read-ms stat (ADR 0224); GUI.Image.TrySetFromRenderTarget provides the single-copy presentation seam for the play pane and compositing phases, D3D11 device-only and Linux EGL-pbuffer headless contexts are implemented; live Windows/Linux evidence remains in the platform sign-off matrix (macOS Metal headless is covered). Game workflow depth (ADR 0225 program close): New Project offers "2D Game" (vm profile) and "3D Game" (native profile) templates whose generated projects carry a starter authored scene, a v19 scene-components schema, and mains conforming to --scene/--scene-watch/--smoke — playable through Run Scene and Play the minute they scaffold (a CTest smoke scaffolds and runs both). Every XenoScape enemy, gem, and upgrade is now scene-authored (the exporter bakes the historical enrichment pass; the runtime injection was deleted), so Studio edits cover the complete gameplay population. 2D objects author the collider convention (box/circle/tile-rect with offsets and triggers) through a live-commit inspector group with canvas wireframe overlays, mirrored by a schema recipe. Bake Navmesh voxel-bakes walkable navigation from the scene using schema-editable nav.* settings and writes a .vnavmsh sidecar that ashfall imports when present — never a scene edit. Package Project runs zanna package as a streamed job. Scene panes float into panels and dock back in both editors. The 3D hierarchy projects through the id-keyed GUI.VirtualTree model (ADR 0226): rows are viewport-provided rather than retained widgets, MAX_EDITOR_NODES is 65,536 with truthful clipping past the ceiling, and multi-select, before/into/after row drops, inline rename, type icons, dimmed pick-locks, and reveal-on-select all ride the virtual contract (2k tightened + 65k slow capacity probes). Deferred from the 2026-07 program (recorded, not silently dropped): the zero-copy GPU-composited viewport (ADR 0224; current GPU targets still read back once per frame), asset-library v2 with palette paging (a data-format program of its own), the sprite-clip library fill and in-inspector preview (the anim.* typed group ships), schema v20 mirror fields (pending the normalization-stability verification the design flagged), scene panes as workbench tab groups (float/dock ships; the tab re-plumb remains) and hidden-start embed windows. The play loop is real (ADR 0225): zanna.project declares run-profile vm|native; Run Scene on a native project chains zanna build into launching the produced binary with --scene --scene-watch (skip-if-fresh makes scene-only iterations ~1s), both example games hot-reload a saved scene after a full parse preflight, saving the watched scene reports that the running game will reload it, and both editors' always-visible Play toolbar items run the game embedded — native projects build first, VM projects launch under the interpreter with the channel injected, and frames stream through a shared-memory channel into the 3D viewport or 2D canvas image with focused mouse/key forwarding and truthful building/waiting/live/exited states. Schema v2 enum/asset fields, the explicit 2D/3D migration assistant, the per-root asset library with tag filtering and import-grid surfacing, and the project material library are present; automatic unattended migration and generalized runtime components are not. |
...
| Terminal | Implemented with limits | PTY-backed shell in OutputPane terminal mode: alternate screen, DECSTBM scroll regions, delayed DECAWM autowrap, DECOM origin-relative addressing, IL/DL/ICH/DCH/ECH, tab stops, cursor visibility, bracketed paste, application cursor keys, DSR/DA replies, SGR 16/256/truecolor + reverse. Coverage is pinned to the vim/less/htop sequence table, not full VT. |
| Source Control | Implemented with limits | Responsive, selection-aware Git controls for async status, stage/unstage, commit, per-path diff, worker-computed/incrementally rendered side-by-side diffs, paged commit history with per-commit files and diffs, virtual generation-tagged rows, and coalesced serialized jobs. Fetch and push stream through bounded Process jobs; pull requires an explicit fast-forward-only, merge, or rebase strategy, with visible operation/conflict state, current/incoming side selection, explicit Stage confirmation, noninteractive merge/rebase Continue, rebase Skip, and merge/rebase Abort. Credentials stay in configured Git helpers, askpass brokers, or SSH agents; Studio disables terminal prompts and never collects secrets. Multi-file conflicts remain exact virtual rows and block ordinary Commit until resolved; a graphical three-pane merge editor and stash management remain absent. |
...
| Cross-platform | Implemented; release evidence pending | The versioned Windows/macOS/Linux matrix covers native pickers, watcher timestamp edges, malformed/large sources, Git credentials/conflicts, vim/less/htop, PTY/ConPTY, huge output, debugger watches, focus/accessibility, and implemented D3D11/EGL/Metal headless scenes. Rows remain pending until the corresponding real platform/manual evidence is recorded; see `docs/platform-signoff.md`. |
...
The 3D hierarchy budget is 65,536 rows with the same truthful clipping
flag past the limit (ADR 0226). Rows project through the id-keyed
`GUI.VirtualTree` model instead of retained widgets: a structural
refresh detaches the projection, rebuilds the model with O(1) edits,
and rebinds once, so scale is bounded by the C model rather than widget
allocation. Multi-select, before/into/after row drops, inline rename,
type icons, dimmed pick-locks, and reveal-on-select all ride the
virtual contract. The 2000-node probe pins unclipped opening and
editability under tightened tripwires with `METRIC:` lines (open now
measures ~1.3 s interpreted where the retained path took ~22 ms per
node), and a 65,536-node slow probe pins the ceiling plus truthful
clipping one row past it. Raising the ceiling surfaced and fixed three
latent quadratic costs — per-insert model rehash oscillation, per-frame
linear parent-row scans in the viewport renderer, and unbounded
parent-target dropdown population (now capped at 512 with a truthful
tooltip; larger scenes reparent through row drops). Expansion capture reads visible rows, so an expanded row under
a collapsed ancestor re-collapses after a structural refresh — the one
recorded fidelity trade against retained capture.
...
available for X, Z, and XZ Move drags and explicit Drop, and the shipped
Vertex mode (persistent toggle or transient V hold) snaps vertex/pivot to
vertex with bounded scans and exact Escape rollback. A pivot-versus-center
toggle and cone/cube handle art remain unimplemented (handles already differ
by tool mode).

...
| `zia_zannastudio_scene_gameplay_preview` | Real Ashfall project loading, exact gameplay eye/FOV across responsive docks, transient terrain composition, schema-v10 retained five-pass post-FX, schema-v13 direct spawn creation with exact metadata/immediate prefab preview/one-step byte-exact undo-redo, schema-v12 1600x900 target framing/matte/camera restoration, finalized high-key luminance, actual PNG capture, Gameplay View recovery, and canonical content isolation. | `zia;zannastudio;scene;requires_display` |
...
| `zia_zannastudio_scene_command_bar` | Real 2D Tools/View and 3D Placement/View popup clicks, active/checkable state refresh, bounded medium-lane command-bar and canvas/viewport height, and wide direct-control restoration. | `zia;zannastudio;scene;requires_display` |
| `zia_zannastudio_scene_workspace_state` | Bounded per-scene canvas/camera/inspector/typed-property-selection/complete-2D-tool-range/transform-tool/shaded-wireframe session persistence and dirty untitled-scene recovery. | `zia;zannastudio;scene;persistence` |
| `zia_zannastudio_scene_component_schema` | Fail-closed project component parsing through schema v13, exact explicit/implicit typed defaults, preview-group completeness and bounds including object draw stacks/paired game-output dimensions, target-compatible creation recipes, target filtering, duplicate rejection, and empty-schema behavior. | `zia;zannastudio;scene` |
| `zia_zannastudio_scene_component_authoring` | Pure structured component/field edits, stable ordering, typed defaults, preview-profile preservation through schema-v12 output frames, schema-v13 starter/edit/target-normalized creation recipes, atomic create/replace, external-conflict rejection, and bounded exact file undo/redo. | `zia;zannastudio;scene` |
...
| `scene_vertex_snap_probe.zia` | Exact runtime source/target candidates and real vertex-to-vertex, pivot-to-vertex, and vertex-to-precise-surface drags, persistent workspace controls, live no-history transforms, one-step release commits, exact Escape/undo restoration, and toolbar containment. |
| `scene_terrain_authoring_probe.zia` | Exact centered grid topology, pure Raise/Lower/Smooth/Flatten behavior, typed-convention rejection, real direct Terrain creation and viewport stroke capture, live mesh/history isolation, one-step commit, exact Escape/undo/redo, flatten/regenerate no-ops and transactions, VSCN round trips, and responsive inspector containment. |
...
| `scene_gameplay_preview_probe.zia` | Real Ashfall mission loading, project-owned camera/environment/post-FX resolution, retained runtime effect count, finalized pale-scene luminance, schema-v13 direct spawn creation with exact metadata/immediate prefab preview/one-step exact history, Gameplay View recovery, schema-v12 1600x900 render-target framing/matte/camera lock and restoration, chrome-free PNG capture, and canonical isolation. |
...
| `scene_command_bar_probe.zia` | Real 2D Tools/View and 3D Placement/View menu clicks, checked-state refresh, medium-lane bar/canvas/viewport bounds, and wide direct-control restoration. |
| `scene_component_schema_probe.zia` | Bounded version-1 through version-13 schema parsing, target lookup, canonical explicit/implicit defaults, complete/ranged preview profiles including object draw stacks and paired game-output dimensions, target-compatible creation recipes, and fail-closed invalid/duplicate behavior. |
| `scene_component_authoring_probe.zia` | Pure component/field create/update/delete/reorder, exact scalar defaults, preview-profile/version preservation through schema-v12 output frames, schema-v13 starter/edit/target-normalized creation recipes, duplicate/last-field rejection, atomic create/replace, external conflicts/reload, and bounded exact file undo/redo. |
| `scene_workspace_state_probe.zia` | Bounded visual-scene, complete 2D tool-range, and typed-property-selection persistence, hostile-field clamping, and untitled scene recovery. |
...
- The 2D/3D scene editors have focused hierarchy, viewport, history,
  ...
  placement/view overflow in 1000-by-700 dock-heavy and 560-by-420 compact
  windows, plus direct controls in 1320-by-760 Scene Layout, and proves every
  reachable action remains inside its command bar. Exact geometry placement has separate
  `scene_vertex_snap_probe` coverage for authoritative runtime vertex
  positions, persistent controls, real vertex/pivot-to-vertex gestures,

...
| `scene_hierarchy_affordances_probe.zia` | Real pointer Show/Hide, exact group visibility undo, mixed-state controls, tab-owned pick locks, virtual-row cues, reorder identity remapping, delete pruning, and responsive action containment. |
| `scene_capacity_probe.zia` | Unclipped 2000-node opening through the virtual hierarchy model, select/reparent/undo usability at size, tightened latency tripwires, and `METRIC:` lines for trend reading (ADR 0226). |
| `scene_capacity_65k_probe.zia` | Unclipped 65,536-row ceiling through the id-keyed model, far-end selection row identity, and truthful clipping one node past the ceiling (slow label). |

...
| `zia_zannastudio_scene_gameplay_preview` | Real Ashfall project loading, exact gameplay eye/FOV across responsive docks, transient terrain composition, schema-v10 retained five-pass post-FX, finalized high-key luminance, actual PNG capture, Gameplay View recovery, and canonical content isolation. | `zia;zannastudio;scene;requires_display` |
...
| `zia_zannastudio_scene_component_schema` | Fail-closed project component parsing through schema v10, exact explicit/implicit typed defaults, preview-group completeness and bounds, target filtering, duplicate rejection, and empty-schema behavior. | `zia;zannastudio;scene` |
| `zia_zannastudio_scene_component_authoring` | Pure structured component/field edits, stable ordering, typed defaults, preview-profile/version preservation through post-FX v10, atomic create/replace, external-conflict rejection, and bounded exact file undo/redo. | `zia;zannastudio;scene` |
...
| `scene_gameplay_preview_probe.zia` | Real Ashfall mission loading, project-owned camera/environment/post-FX resolution, retained runtime effect count, finalized pale-scene luminance and PNG capture, responsive-layout eye anchoring, Gameplay View recovery, and canonical isolation. |
...
| `scene_component_schema_probe.zia` | Bounded version-1 through version-10 schema parsing, target lookup, canonical explicit/implicit defaults, complete/ranged preview profiles, and fail-closed invalid/duplicate behavior. |
| `scene_component_authoring_probe.zia` | Pure component/field create/update/delete/reorder, exact scalar defaults, preview-profile/version preservation through post-FX v10, duplicate/last-field rejection, atomic create/replace, external conflicts/reload, and bounded exact file undo/redo. |
